...
export class EcosystemJobListResultEntity {
  constructor(private readonly raw: RawEcosystemJobPost) {}

  getProperties(
    transform?: (job: EcosystemJobListResult) => EcosystemJobListResult,
  ): EcosystemJobListResult {
    const jobpost = this.raw;
    const { organization, project, tags } = jobpost;
    const reviews =
    ...
    if (transform) {
      return new EcosystemJobListResult(transform(processed));
    } else {
      return new EcosystemJobListResult(processed);
    }
  }
}
